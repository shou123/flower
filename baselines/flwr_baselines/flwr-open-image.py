...
def load_datasets(strategy: str, alpha:float) -> Tuple[List[DataLoader], List[DataLoader], DataLoader]:
    ...
    data_path = "/home/labuser/shiyue/flower/open_image_dataset"
...
# )
    ...
    # max_samples = 80,
    shuffle=True,
    dataset_name="openimagesv6",
    label_types=["detections"],
    persistent=True,
    dataset_dir=data_path
)

    for sample in trainset.take(5):
        print(sample.filepath)

    testset = foz.load_zoo_dataset(
    "open-images-v6",
    split="test",
    # max_samples = 80000,
    shuffle=True,
    ...
    data_path+"/patches",
    transform=transforms.Compose([
        transforms.Resize((256,256)),  # Ensure uniform size of 256x256
        transforms.ToTensor(),
        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    ])
)
    ...
    for sample in trainset:
        ...
        for detection in detections:
            labels.append(detection.labels)
    # num_classes = len(trainDataset.classes)
    num_classes = len(set(labels))
    class_indices = {i: np.where(np.array(labels) == i)[0] for i in range(num_classes)}
