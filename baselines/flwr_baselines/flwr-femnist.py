...
# FEMNIST Dataset Loader (Assumes LEAF dataset format)
...
def load_femnist_data(data_path: str) -> Tuple[List[FEMNISTDataset], FEMNISTDataset]:
    """
    Loads FEMNIST data from LEAF dataset structure.
    """
    all_train_data = []
    all_train_labels = []
    all_test_data = []
    ...
    transform = transforms.Compose([
        transforms.Grayscale(num_output_channels=1),  # Convert grayscale to 3 channels
        transforms.ToTensor(),
        # transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    ])

 # Load data from all JSON files
    for json_file in os.listdir(os.path.join(data_path, "train")):
        client_path = os.path.join(data_path, "train", json_file)
        with open(client_path, "r") as f:
            client_data = json.load(f)
            user_data = client_data["user_data"]
            
            for user in user_data:
                all_train_data.extend(user_data[user]["x"])
                all_train_labels.extend(user_data[user]["y"])

    for json_file in os.listdir(os.path.join(data_path, "test")):
        test_path = os.path.join(data_path, "test", json_file)
        with open(test_path, "r") as f:
            client_data = json.load(f)
            user_data = client_data["user_data"]
            
            for user in user_data:
                all_test_data.extend(user_data[user]["x"])
                all_test_labels.extend(user_data[user]["y"])
    ...
    trainset = FEMNISTDataset(all_train_data,all_train_labels,transform=transform)
    testset = FEMNISTDataset(all_test_data,all_test_labels,transform=transform)

    return trainset,testset

def load_datasets(strategy: str, alpha: float, data_path: str) -> Tuple[List[DataLoader], DataLoader]:
    """
    Loads FEMNIST datasets, partitions data according to strategy, and returns DataLoaders.
    """

    # Load FEMNIST dataset
    trainset,testset = load_femnist_data(data_path)
...
trainloaders, valloaders, testloader = load_datasets(args.strategy,args.alpha,data_path='/home/labuser/shiyue/flower/leaf/data/femnist/data')
...
class Net(nn.Module):
    ...
    def forward(self, xb):
        # print(f"Shape after convolutional layers and pooling: {xb.shape}")
...
class Distance_selection_no_layer(Strategy):
    ...
    def __init__(
        ...
        *,
        ...
        self.last_client_weights = {}  # Store the most recent weights for each client
    ...
    def select_next_round_clients(self, client_distances, fraction_fit):
        num_clients_to_select = int(len(client_distances) * fraction_fit)
        sorted_clients = sorted(client_distances.items(), key=lambda item: item[1], reverse=True)
    ...
    def aggregate_fit(
        ...
        if not self.accept_failures and failures:
            print(f"Round {server_round}: Failures encountered and not accepted.")
            return None, {}
        ...
        for client_id, client_weights in self.last_client_weights.items():
            distance = self.calculate_client_distance(client_id, client_weights, parameters_to_ndarrays(parameters_aggregated))
        ...
        for client_id, distance in all_client_distances.items():
            print(f"Round: {server_round}, Client ID: {client_id}, Distance: {distance}")
        ...
        sorted_clients = sorted(self.all_client_distances.items(), key=lambda item: item[1], reverse=True)
        print(f"Client distances: {self.all_client_distances}")
        print(f"Sorted clients {sorted_clients}")
        print(f"Selected clients for next round: {self.next_round_clients}")
    ...
    def calculate_client_distance(self, client_id, client_weights, server_weights):
        total_distance = 0
        for client_layer, server_layer in zip(client_weights, server_weights):
...
#Test Strategy
strategy = Distance_selection_no_layer(
